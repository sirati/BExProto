...
import static de.sirati97.bex_proto.v2.service.modular.internal.BouncyCastleHelper.*;
...
public class EncryptionModule extends InternalModule<EncryptionModule.EncryptionData> implements IModuleHandshake, IPacketHandler {
    ...
    @Override
    public EncryptionData createData(ModularService connection) {
    ...
    private void closeWithError(String text, Throwable t, ModularService con) {
        con.getLogger().error(text+" :", t);
        _closeWithError(text, con);
    }

    private void closeWithError(String text, ModularService con) {
    ...
    private void onServerPublicKey(byte[] key, ModularService connection) {
    ...
    private void onTrustServerAnswer(byte[] dataVector, ModularService connection) {
    ...
    private void onSecretKey(ModularService connection, byte[] dataKey, byte[] dataVector) {
        EncryptionData encryptionData = getModuleData(connection);
        encryptionData.callback.yield(YieldCause.PacketReceived);
    ...
    private void onSuccess(ModularService connection) {
        EncryptionData encryptionData = getModuleData(connection);
        encryptionData.callback.yield(YieldCause.PacketReceived);
    ...
    private void onClientPublicKey(byte[] key, ModularService connection) {
        EncryptionData encryptionData = getModuleData(connection);
        encryptionData.callback.yield(YieldCause.PacketReceived);
        PublicKey publicKey;
        try {
    ...
    private void onTrustServer(byte[] dataVector, ModularService connection) {
        EncryptionData encryptionData = getModuleData(connection);
        encryptionData.callback.yield(YieldCause.PacketReceived);
    ...
    private void onRequestSecretKey(ModularService connection) {
        EncryptionData encryptionData = getModuleData(connection);
        encryptionData.callback.yield(YieldCause.PacketReceived);
        if (encryptionData.remotePublicKey == null || encryptionData.vectorPartClient == null) {
            closeWithError("Unexpected Error!", connection);
    ...
    private void onSecretKeyConfirm(ModularService connection, byte[] dataKey, byte[] dataVector) {
        EncryptionData encryptionData = getModuleData(connection);
        encryptionData.callback.yield(YieldCause.PacketReceived);
        if (encryptionData.remotePublicKey == null || encryptionData.vectorPartClient == null || encryptionData.vectorPartServer == null || encryptionData.secretKey == null) {
            closeWithError("Unexpected Error!", connection);
    ...
    private void onError(String message, ModularService connection) {
        getModuleData(connection).callback.error(new HandshakeRejectedException(message));
    }
    private void onError(Throwable t, ModularService connection) {
