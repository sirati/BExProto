...
public class PacketManager {
    public static ReceivedPacket extract(PacketDefinition definition, CursorByteBuffer buf) {
        ...
        for (int i=0;i<definition.getArgumentLength();i++) {
    ...
    public static ByteBuffer createStream(Packet packet) {
        ...
        for (int i=0;i<packet.getArgumentLength();i++) {
            IType type = packet.getType(i);
            Object tempObj = packet.get(i);
            type.getEncoder().encodeObj(tempObj, buffer, i+1<packet.getArgumentLength());
        }
        return buffer;
    }
}
